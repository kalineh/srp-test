...
public static class CustomRenderUtils
{
...
public class CustomRenderPipeline : RenderPipeline
{
    ...
    private static readonly ShaderPassName m_ForwardPassName = new ShaderPassName("CustomRenderForward");
    ...
    public CustomRenderPipeline(CustomRenderPipelineAsset asset)
    {
        ...
        m_BlitQuad = CustomRenderUtils.CreateQuadMesh(false);
    ...
    public override void Render(ScriptableRenderContext context, Camera[] cameras)
    {
        ...
        foreach (Camera camera in cameras)
        {
            ...
            if (CustomRenderUtils.HasFlag(frameRenderingConfiguration, FrameRenderingConfiguration.DepthPass))
    ...
    private void ShadowPass(VisibleLight[] visibleLights, ref ScriptableRenderContext context, ref LightData lightData)
    {
        if (m_Asset.AreShadowsEnabled() && lightData.mainLightIndex != -1)
        {
            ...
            {
                if (!CustomRenderUtils.IsSupportedShadowType(mainLight.lightType))
                {
    ...
    {
        var opaqueDrawSettings = new DrawRendererSettings(m_CurrCamera, m_ForwardPassName);
        opaqueDrawSettings.sorting.flags = SortFlags.CommonOpaque;
    ...
    private void AfterOpaque(ref ScriptableRenderContext context, FrameRenderingConfiguration config)
    {
        ...
        if (CustomRenderUtils.HasFlag(config, FrameRenderingConfiguration.BeforeTransparentPostProcess))
        {
        ...
        if (CustomRenderUtils.HasFlag(config, FrameRenderingConfiguration.DepthCopy))
        {
    ...
    {
        if (!CustomRenderUtils.HasFlag(config, FrameRenderingConfiguration.PostProcess))
            return;
    ...
    {
        var transparentSettings = new DrawRendererSettings(m_CurrCamera, m_ForwardPassName);
    ...
    private void SetupFrameRendering(out FrameRenderingConfiguration configuration, bool stereoEnabled)
    {
        ...
        if (msaaEnabled)
        {
            ...
            intermediateTexture = intermediateTexture || !CustomRenderUtils.PlatformSupportsMSAABackBuffer();
        }
    ...
    private void SetupIntermediateResources(FrameRenderingConfiguration renderingConfig, ref ScriptableRenderContext context)
    {
        ...
        msaaSamples = (CustomRenderUtils.HasFlag(renderingConfig, FrameRenderingConfiguration.Msaa)) ? msaaSamples : 1;
        m_CurrCameraColorRT = BuiltinRenderTextureType.CameraTarget;

        if (CustomRenderUtils.HasFlag(renderingConfig, FrameRenderingConfiguration.IntermediateTexture))
        {
            if (CustomRenderUtils.HasFlag(renderingConfig, FrameRenderingConfiguration.Stereo))
    ...
    private void SetupIntermediateResourcesSingle(CommandBuffer cmd, FrameRenderingConfiguration renderingConfig, int msaaSamples)
    {
        ...
        if (m_RequiredDepth)
        {
            ...
            if (CustomRenderUtils.HasFlag(renderingConfig, FrameRenderingConfiguration.DepthCopy))
        ...
        if (CustomRenderUtils.HasFlag(renderingConfig, FrameRenderingConfiguration.BeforeTransparentPostProcess))
    ...
    private int GetMainLight(VisibleLight[] visibleLights)
    {
        ...
        for (int i = 0; i < totalVisibleLights; ++i)
        {
            ...
            if (shadowsEnabled && currLight.light.shadows != LightShadows.None && CustomRenderUtils.IsSupportedShadowType(currLight.lightType))
                return i;
    ...
    private void BeginForwardRendering(ref ScriptableRenderContext context, FrameRenderingConfiguration renderingConfig)
    {
        ...
        if (CustomRenderUtils.HasFlag(renderingConfig, FrameRenderingConfiguration.IntermediateTexture))
        {
            ...
            if (m_RequiredDepth && !CustomRenderUtils.HasFlag(renderingConfig, FrameRenderingConfiguration.DepthPass))
                depthRT = m_DepthRT;
        }
    ...
    private void EndForwardRendering(ref ScriptableRenderContext context, FrameRenderingConfiguration renderingConfig)
    {
        ...
        else if (CustomRenderUtils.HasFlag(renderingConfig, FrameRenderingConfiguration.IntermediateTexture))
        {
            // If PostProcessing is enabled, it is already blit to CameraTarget.
            if (!CustomRenderUtils.HasFlag(renderingConfig, FrameRenderingConfiguration.PostProcess))
    ...
    {
        if (CustomRenderUtils.HasFlag(renderingConfig, FrameRenderingConfiguration.DefaultViewport))
        {
